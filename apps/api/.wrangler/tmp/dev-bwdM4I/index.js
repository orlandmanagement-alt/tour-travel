...
// .wrangler/tmp/bundle-p35mvs/checked-fetch.js
  ...
  static async handle(request, env, path) {
    try {
      ...
      if (request.method === "POST") {
        ...
        if (path === "/api/master/locations") {
          const { results } = await env.DB.prepare("INSERT INTO master_locations (city_code, city_name, province) VALUES (?, ?, ?) RETURNING *").bind(body.city_code || "NEW", body.city_name, body.province || "Unknown").all();
          return sendResponse(results[0]);
        }
        if (path === "/api/master/addons") {
          const { results } = await env.DB.prepare("INSERT INTO master_addons (addon_name, location_id, charge_type, default_price) VALUES (?, ?, ?, ?) RETURNING *").bind(body.addon_name, body.location_id, body.charge_type, body.default_price).all();
          return sendResponse(results[0]);
        }
      }
...
var TourController = class {
  ...
  static async generateAutoCode(env, url) {
    try {
...
var CustomTripController = class {
  static {
    __name(this, "CustomTripController");
  }
...
// .wrangler/tmp/bundle-p35mvs/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-p35mvs/middleware-loader.entry.ts
